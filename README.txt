...
It consists of two seperate projects. The server is running on a computer with a static ip and listening on a specified port for incoming connections. The client can be launched on any machine and it connects to the server in order to join the server lobby. In the lobby clients can exchange chat messages and arrange games.
